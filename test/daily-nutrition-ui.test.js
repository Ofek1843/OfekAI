...
test("natural portions, estimates, partial success and editable corrections are wired", () => {
  ...
  assert.match(client, /writeDailyDraft\(uid, dateKey, log, draftToken\)/);
  assert.match(client, /readDailyDraft\(state\.user\.uid, state\.dateKey\)/);
  assert.match(client, /saveDailyLogButton/);
  assert.match(html, /id="saveDailyLogButton"/);
