...
function readExistingMeal(meal) { const option = meal.options?.[0] || meal; return { id: meal.mealId || meal.id || `saved-${Math.random()}`, title: meal.name || option.name || "Meal", image: option.image || meal.image || "/images/food-placeholder.png", foods: option.baseFoods || option.foods || [], servings: Math.max(.25, Math.min(8, number(option.servings) || 1)) }; }
