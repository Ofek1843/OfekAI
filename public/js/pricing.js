...
$("#upgradeButton").addEventListener("click",async()=>{
  ...
  try{const reference=doc(db,"users",activeUser.uid,"waitlists","pro");const existing=await getDoc(reference);await setDoc(reference,{email:activeUser.email||"",plannedPriceIls:10,status:"interested",source:"pricing-page",updatedAt:serverTimestamp(),...(existing.exists()?{}:{createdAt:serverTimestamp()})},{merge:true});showJoined();$("#pricingStatus").textContent=he?copy.success:"You're on the wishlist. No card was entered and no charge was made.";}catch(error){console.error("Wishlist signup failed:",error);button.disabled=false;button.textContent=he?copy.choose:"Join the Pro wishlist";$("#pricingStatus").classList.add("error");$("#pricingStatus").textContent=he?"לא הצלחנו לשמור את ההצטרפות. נסה שוב.":"Could not join the wishlist. Please try again.";}
});
