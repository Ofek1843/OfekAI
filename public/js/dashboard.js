...
function showPushStatus(text, isError = false) {
...
function initPushNotifications() {
  ...
  const label = $("#enablePushButtonLabel");
  if (!button || !label) return;

  if (pushPermissionState() === "granted") {
    label.textContent = ui.pushOn;
  }

  listenForForegroundPush(({ title, body }) => {
    if (title || body) showPushStatus([title, body].filter(Boolean).join(" — "));
  });

  button.addEventListener("click", async () => {
    button.disabled = true;
    label.textContent = ui.pushEnabling;
    ...
    if (result.ok) {
      label.textContent = ui.pushOn;
      button.disabled = false;
      showPushStatus(ui.pushOn);
      return;
    }

    button.disabled = false;
    label.textContent = ui.pushEnable;

    const reasonText = {
      denied: ui.pushDenied,
      not_configured: ui.pushNotConfigured,
      unsupported: ui.pushUnsupported,
      signed_out: ui.pushSignedOut
    }[result.reason] || ui.pushError;

    showPushStatus(reasonText, true);
  });
}
