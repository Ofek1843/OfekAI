...
function hasMp4Signature(buffer) {
  ...
  if (boxSize < contentOffset + 8 || boxSize > buffer.length) return false;
  const knownBrands = new Set([
    "M4A ", "M4B ", "isom", "iso2", "iso3", "iso4", "iso5", "iso6",
    "mp41", "mp42", "avc1", "dash", "MSNV", "qt  "
  ]);
  const brands = [buffer.subarray(contentOffset, contentOffset + 4).toString("ascii")];
  for (let offset = contentOffset + 8; offset + 4 <= boxSize; offset += 4) {
    brands.push(buffer.subarray(offset, offset + 4).toString("ascii"));
  }
  return brands.some((brand) => knownBrands.has(brand));
}
