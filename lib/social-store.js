...
async function listConversations(uid) {
  ...
  const profiles = await profilesFor(db, snapshot.docs.map((item) => item.data().otherUid));
  return snapshot.docs.map((item) => ({ id: item.id, ...item.data(), profile: profiles.get(item.data().otherUid) || { uid: item.data().otherUid } }));
}
...
async function graphFromSource(db, uid, input) {
  ...
  if (sourceId === "body-weight" || sourceId === "weight-change") {
    ...
    return {
      ...
      points: snapshot.docs.map((item) => ({ date: cleanString(item.data().date, 24), value: Number(item.data().weight) })).reverse()
    };
  }
...
async function getArtifact(uid, artifactId) {
  ...
  if (artifact.ownerUid !== uid && ["workout", "nutrition"].includes(artifact.type)) {
    ...
    if (artifact.type === "workout") {
      ...
      const available = ownEquipment.map((item) => cleanString(item, 60).toLowerCase());
      const required = (artifact.snapshot?.equipment || []).map((item) => cleanString(item, 60)).filter(Boolean);
      if (available.length) {
        const missing = required.filter((item) => !available.includes(item.toLowerCase()));
  ...
  return { id: snapshot.id, ...artifact, unavailable: Boolean(artifact.revokedAt), compatibilityWarnings };
}
