...
const { safeVoiceMetadata } = require("./voice-message-domain");
const { normalizeMusicLink } = require("./music-link-domain");
const { normalizeEquipment } = require("./workout-validator");
const { minimalSnapshot, normalizeReport, reportId } = require("./social-report-domain");
...
function socialFirestore() {
  ...
  try {
    cachedDb = getFuelPhysiqueFirestore();
  } catch {
...
async function getConversation(uid, conversationId) {
  ...
  if (isDeletedConversationForSurvivor(snapshot.data(), uid, otherUid)) {
    return { id: snapshot.id, ...snapshot.data(), profile: deletedProfile(otherUid) };
  }
...
function isDeletedConversationForSurvivor(conversation = {}, uid, otherUid) {
  const deleted = Array.isArray(conversation.deletedParticipantUids) ? conversation.deletedParticipantUids : [];
  return conversation.status === "deleted_participant" && deleted.includes(otherUid) && !deleted.includes(uid);
}

function deletedProfile(uid) {
  return { uid, deleted: true, displayName: "Deleted user", username: "", photoURL: "", badges: [] };
}
...
async function listConversations(uid) {
  ...
  for (const item of snapshot.docs) {
    ...
    if (item.data()?.status === "deleted_participant") {
  ...
  const profiles = await profilesFor(db, visibleDocs.filter((item) => item.data()?.status !== "deleted_participant").map((item) => item.data().otherUid));
  return visibleDocs.map((item) => ({
    id: item.id,
    ...item.data(),
    profile: item.data()?.status === "deleted_participant"
      ? deletedProfile(item.data().otherUid)
      : profiles.get(item.data().otherUid) || { uid: item.data().otherUid }
  }));
}
...
async function listMessages(uid, conversationId, cursor) {
  ...
  const otherUid = conversation.data().participants.find((participant) => participant !== uid);
  if (!isDeletedConversationForSurvivor(conversation.data(), uid, otherUid)) {
    await assertAcceptedFriendsRead(db, uid, otherUid);
  }
...
function messagePreview(message) {
  ...
  if (message.type === "music_link") return "Shared workout music";
...
async function sendMusicMessage(uid, conversationId, input = {}) {
  const db = socialFirestore();
  const music = normalizeMusicLink(input);
  const messageId = stableMessageId(uid, input.clientId);
  const conversationRef = db.doc(`conversations/${cleanString(conversationId, 300)}`);
  const messageRef = conversationRef.collection("messages").doc(messageId);
  return db.runTransaction(async (transaction) => {
    ...
    return writeMessageTransaction(db, transaction, {
      ...
      conversation,
      messageRef,
      message: { type: "music_link", music, senderUid: uid, schemaVersion: 1, createdAt: Timestamp.now() }
    });
  });
}

async function prepareVoiceMessage(uid, conversationId, clientId) {
  const db = socialFirestore();
  const cleanConversationId = cleanString(conversationId, 300);
  const conversation = await getConversation(uid, cleanConversationId);
  if (conversation.status !== "active") throw new SocialError("conversation_inactive", "This conversation is read-only.", 403);
  const messageId = stableMessageId(uid, clientId);
  const existing = await db.doc(`conversations/${cleanConversationId}/messages/${messageId}`).get();
  if (existing.exists) {
    if (existing.data()?.senderUid !== uid || existing.data()?.type !== "voice") {
      throw new SocialError("message_conflict", "This voice-message retry could not be verified.", 409);
    }
    return { duplicate: true, message: documentData(existing), messageId };
  }
  return { duplicate: false, messageId };
}

async function sendVoiceMessage(uid, conversationId, input = {}) {
  const db = socialFirestore();
  const voice = safeVoiceMetadata(input.voice);
  const messageId = stableMessageId(uid, input.clientId);
  const conversationRef = db.doc(`conversations/${cleanString(conversationId, 300)}`);
  const messageRef = conversationRef.collection("messages").doc(messageId);
  return db.runTransaction(async (transaction) => {
    ...
    return writeMessageTransaction(db, transaction, {
      ...
      conversation,
      messageRef,
      message: { type: "voice", voice, senderUid: uid, schemaVersion: 1, createdAt: Timestamp.now() }
    });
  });
}

async function getMessageForParticipant(uid, conversationId, messageId) {
  const db = socialFirestore();
  const cleanConversationId = cleanString(conversationId, 300);
  await getConversation(uid, cleanConversationId);
  const message = await db.doc(`conversations/${cleanConversationId}/messages/${cleanString(messageId, 100)}`).get();
  if (!message.exists) throw new SocialError("message_not_found", "Message not found.", 404);
  return documentData(message);
}

async function getOwnedMessage(uid, conversationId, messageId) {
  const db = socialFirestore();
  const cleanConversationId = cleanString(conversationId, 300);
  const conversation = await db.doc(`conversations/${cleanConversationId}`).get();
  if (!conversation.exists || !(conversation.data()?.participants || []).includes(uid)) throw new SocialError("conversation_not_found", "Conversation not found.", 404);
  const message = await db.doc(`conversations/${cleanConversationId}/messages/${cleanString(messageId, 100)}`).get();
  if (!message.exists) throw new SocialError("message_not_found", "Message not found.", 404);
...
async function deleteMessage(uid, conversationId, messageId) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (!message.exists) throw new SocialError("message_not_found", "Message not found.", 404);
    if (message.data()?.senderUid !== uid) throw new SocialError("forbidden", "You can only delete your own messages.", 403);
    const update = { text: "", deletedAt: timestampNow(), deletedBy: uid };
    if (message.data()?.type === "voice") {
      const voice = message.data()?.voice || {};
      update.voice = {
        durationMs: Number(voice.durationMs || 0),
        mimeType: cleanString(voice.mimeType, 120),
        sizeBytes: Number(voice.sizeBytes || 0),
        unavailable: true
      };
    }
    if (message.data()?.type === "music_link") {
      update.music = { provider: cleanString(message.data()?.music?.provider, 40), title: "", unavailable: true };
    }
...
async function reportContent(uid, input = {}) {
  const report = normalizeReport(input);
  const db = socialFirestore();
  let snapshot;
  if (report.targetType === "user") {
    const profile = await getPublicProfile(uid, report.targetId);
    snapshot = minimalSnapshot({ targetType: "user", targetId: report.targetId, value: profile });
  } else if (report.targetType === "message") {
    const [conversationId, messageId] = report.targetId.split(":", 2);
    if (!conversationId || !messageId) throw new SocialError("invalid_report", "Choose a valid message to report.", 400);
    await getConversation(uid, conversationId);
    const message = await db.doc(`conversations/${cleanString(conversationId, 300)}/messages/${cleanString(messageId, 100)}`).get();
    if (!message.exists) throw new SocialError("message_not_found", "Message not found.", 404);
    snapshot = minimalSnapshot({ targetType: "message", targetId: report.targetId, value: message.data() });
  } else {
    const artifact = await getArtifact(uid, report.targetId);
    snapshot = minimalSnapshot({ targetType: "artifact", targetId: report.targetId, value: artifact });
  }
  const id = reportId(uid, report);
  const ref = db.doc(`socialReports/${id}`);
  const existing = await ref.get();
  if (existing.exists && existing.data()?.status !== "closed") return { reportId: id, duplicate: true, targetType: report.targetType, reason: report.reason };
  await ref.set({
    reporterUid: uid,
    ...
    snapshot,
    status: "open",
    createdAt: existing.exists ? existing.data()?.createdAt || timestampNow() : timestampNow(),
    updatedAt: timestampNow()
  }, { merge: true });
  return { reportId: id, duplicate: false, targetType: report.targetType, reason: report.reason };
}
