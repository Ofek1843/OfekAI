    ...
    for (const candidate of existing) {
      ...
      if (fitsSessionBudget(candidate.session, profile) && isHardMaximumSafe(next, profile)) {
        return mutationResult(next, profile, {
          ...
          exerciseName: candidate.exercise.name, exerciseIndex: candidate.exerciseIndex, setsDelta: 1, muscle: targetMuscle
        });
      }
    ...
    for (const candidateSession of sessions) {
      ...
      if (fitsSessionBudget(candidateSession.session, profile) && isHardMaximumSafe(next, profile)) {
        return mutationResult(next, profile, {
          ...
          exerciseName: exercise.name, exerciseIndex: candidateSession.session.exercises.length - 1, setsDelta: exercise.sets, muscle: targetMuscle
        });
      }
  ...
  for (const candidate of removable) {
    ...
    if (sets >= MIN_MEANINGFUL_SETS && candidate.session.exercises.length > 1) {
      ...
      return mutationResult(next, profile, {
        ...
        exerciseName: candidate.exercise.name, exerciseIndex: candidate.exerciseIndex, setsDelta: -sets, muscle: targetMuscle
      });
    }
  }
