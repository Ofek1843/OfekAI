...
function createSocialRouter({ authenticate, rateLimiters = {}, authorizeAdmin = () => false, notifications = null }) {
  ...
  router.post("/reports", limit("reports"), asyncRoute(async (req, res) => {
    res.status(201).json(await social.reportContent(req.socialUser.uid, req.body || {}));
  }));
