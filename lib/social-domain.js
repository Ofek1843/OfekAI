...
const { calculateWeeklyVolume, slugifyExerciseId } = require("./workout-volume");
...
function sanitizeExercise(exercise = {}) {
  ...
  const exerciseId = slugifyExerciseId(exercise.exerciseId || exercise.id || exercise.demoName || name);
...
function sanitizeWorkoutSnapshot(source = {}, creatorUsername = "") {
  ...
  const volume = calculateWeeklyVolume(normalizedPlan);
  ...
  ].map((item) => cleanString(item, 60)).filter(Boolean))].slice(0, 20);
  return {
...
function sanitizeRecordSnapshot(record = {}, creatorUsername = "") {
  ...
  return {
    schemaVersion: ARTIFACT_SCHEMA_VERSION,
    exerciseId: slugifyExerciseId(record.exerciseId || exerciseName),
    exerciseName,
...
function sanitizeCompletedWorkoutSnapshot(log = {}, creatorUsername = "") {
  const exercises = (Array.isArray(log.exercises) ? log.exercises : []).slice(0, 30).map((exercise) => ({
    exerciseId: slugifyExerciseId(exercise.exerciseId || exercise.name),
...
function buildWorkoutCopy(snapshot, attribution, sourceArtifactId) {
  ...
  plan.weeklyVolume = calculateWeeklyVolume(plan);
