...
const { applyProgramSplitIdentity, classifySessionSplit } = require("./workout-program-identity");
const {
...
// Free-text limitations are still deliberately conservative: only an
// explicit pain/injury/restriction phrase turns a body-part mention into a
// hard exclusion. This prevents harmless notes such as "chest-focused" from
// silently removing a muscle while making the local fallback safe to use.
const LIMITATION_SIGNAL = /(?:injur|pain|cannot|can't|can not|avoid|forbidden|do not train|don't train|not train|אסור|פציע|כאב|לא יכול|להימנע|לא לאמן)/iu;
const LIMITED_MUSCLE_ALIASES = Object.freeze({
  chest: ["chest", "pectoral", "pecs", "חזה"],
  back: ["back", "lat", "lats", "גב"],
  delts: ["shoulder", "shoulders", "delt", "delts", "כתף", "כתפיים"],
  biceps: ["biceps", "bicep", "דו-ראשי", "יד קדמית"],
  triceps: ["triceps", "tricep", "תלת-ראשי", "יד אחורית"],
  quads: ["quad", "quads", "quadriceps", "ירך קדמית"],
  hamstrings: ["hamstring", "hamstrings", "ירך אחורית"],
  glutes: ["glute", "glutes", "ישבן"],
  calves: ["calf", "calves", "שוקיים", "שוק"],
  core: ["core", "abs", "abdominals", "בטן", "ליבה"]
});

function limitedMusclesFromText(limitations) {
  const text = String(limitations || "").trim().toLocaleLowerCase();
  if (!text || text === "none" || !LIMITATION_SIGNAL.test(text)) return new Set();
  return new Set(Object.entries(LIMITED_MUSCLE_ALIASES)
    .filter(([, aliases]) => aliases.some((alias) => text.includes(alias.toLocaleLowerCase())))
    .map(([muscle]) => muscle));
}

const FOUNDATION_IDS_BY_MUSCLE = Object.freeze({
  chest: ["barbell-bench-press", "dumbbell-bench-press", "machine-chest-press"],
  back: ["barbell-row", "dumbbell-row", "lat-pulldown", "seated-cable-row"],
  quads: ["barbell-squat", "barbell-front-squat", "hack-squat", "leg-press"],
  hamstrings: ["romanian-deadlift", "conventional-deadlift", "lying-leg-curl", "seated-leg-curl"],
  glutes: ["barbell-hip-thrust", "dumbbell-hip-thrust", "romanian-deadlift"],
  delts: ["barbell-shoulder-press", "dumbbell-shoulder-press", "machine-shoulder-press", "arnold-press"]
});

function listRank(list, id) {
  const index = (list || []).indexOf(id);
  return index < 0 ? Number.MAX_SAFE_INTEGER : index;
}

function collectCandidates(equipment, { skillForwardCalisthenics = false, preferFoundations = false, experience = "intermediate", excludedMuscles = new Set() } = {}) {
  ...
  for (const [muscle, candidates] of candidatesByMuscle) {
    candidates.sort((left, right) => {
      ...
      if (preferFoundations) {
        const foundations = FOUNDATION_IDS_BY_MUSCLE[muscle] || [];
        const leftFoundation = listRank(foundations, left.id);
        const rightFoundation = listRank(foundations, right.id);
        if (leftFoundation !== rightFoundation) return leftFoundation - rightFoundation;
      }
...
function makeExercise(candidate, sets, limitations, language) {
  ...
  const prescriptions = {
    planche: ["8-15 sec", 150, "1-2"],
    "straddle-planche": ["8-15 sec", 150, "1-2"],
    "front-lever": ["8-15 sec", 150, "1-2"],
    handstand: ["20-40 sec", 120, "1-2"],
    "l-sit": ["15-30 sec", 120, "1-2"],
    "one-arm-pull-up": ["2-5", 180, "1-2"],
    "muscle-up": ["3-6", 180, "1-2"],
    "ring-muscle-up": ["3-6", 180, "1-2"],
    "handstand-push-up": ["4-8", 150, "1-2"],
    "typewriter-pull-ups": ["4-8", 150, "1-2"],
    "front-lever-raise": ["4-8", 150, "1-2"],
    "dragon-flag": ["5-8", 120, "1-2"]
  };
  const [reps, restSeconds, rir] = prescriptions[candidate.id] || ["8-12", 90, "1-3"];
  return {
...
function sessionNameForSplit(session, language) {
  const split = classifySessionSplit(session);
  const labels = language === "he"
    ? { full: "פול באדי", upper: "פלג גוף עליון", lower: "פלג גוף תחתון", legs: "רגליים", push: "פוש", pull: "פול" }
    : { full: "Full Body", upper: "Upper Body", lower: "Lower Body", legs: "Legs", push: "Push", pull: "Pull" };
  return labels[split] || (language === "he" ? "אימון אישי" : "Personalized Session");
}
...
function buildLocalExerciseReplacement({
  ...
  if (limitedMusclesFromText(limitations).has(targetMuscle)) return null;
  ...
  for (const rawId of getEnabledPublicExerciseIds()) {
    ...
    if (limitedMusclesFromText(limitations).has(targetMuscle)) continue;
  ...
  return {
    ...
    // Keep the same universal four-working-set ceiling as the production
    // validator and Weekly Muscle Volume adjustment endpoint.
    sets: Math.min(4, Math.max(1, Math.round(Number(currentExercise?.sets) || fallback.sets))),
...
function buildLocalWorkoutProgram({
  ...
  const normalizedStyle = String(trainingStyle).toLowerCase();
  const normalizedExperience = String(experience).toLowerCase();
  const excludedMuscles = limitedMusclesFromText(limitations);
  const skillForwardCalisthenics = normalizedStyle === "calisthenics" && ["advanced", "professional"].includes(normalizedExperience);
  const preferFoundations = normalizedStyle === "gym" && ["advanced", "professional"].includes(normalizedExperience);
  const { candidatesByMuscle } = collectCandidates(equipment, { skillForwardCalisthenics, preferFoundations, experience, excludedMuscles });
  ...
  placements
    ...
    .forEach((placement, index) => {
      ...
      if (Number(sessionDuration) < 45) exercise.restSeconds = exercise.restSeconds > 90 ? exercise.restSeconds : 60;
      else if (Number(sessionDuration) >= 75) exercise.restSeconds = Math.max(120, exercise.restSeconds);
  ...
  for (const session of sessions) {
    session.name = sessionNameForSplit(session, language);
  }

  return applyProgramSplitIdentity({
