...
function notificationCopy({ type, locale, senderName, text, sessionName, eventSeed }) {
  ...
  if (type === "workout_checkin") {
    const name = plainTextPreview(sessionName || (he ? "האימון שלך" : "your workout"), 72);
    return {
      title: he ? `איך היה ${name}?` : `How did ${name} go?`,
      body: he ? "צ׳ק־אין קצר על האימון — כמה שניות וזהו." : "Quick workout check-in — just a few seconds."
    };
  }
  const en = [
