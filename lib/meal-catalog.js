...
// filename. The UI prefers the production WebP rendition, with the original
// PNG/JPEG kept as a compatibility fallback — see docs/meal-images-plan.md.

const fs = require("fs");
const path = require("path");
const { resolveFoodImage, FOOD_PLACEHOLDER_IMAGE } = require("./food-image-map");
const everydayMeals = require("./everyday-meals");
const expansionMeals = require("./catalog-expansion-meals");
...
const MEALS = [
  ...expansionMeals,
  ...everydayMeals,
  // Israeli supermarket staples: simple, recognizable combinations with a
  // dedicated local plate image for the meal card.
  { id: "israeli-breakfast-plate", en: "Israeli Breakfast Plate", he: "צלחת ארוחת בוקר ישראלית", slots: ["breakfast"], diet: "vegetarian", items: [["eggs", 100], ["cottage-cheese", 120], ["pita", 60], ["tomato", 100], ["cucumber", 100], ["olives", 20]], prepEn: "Serve eggs, cottage cheese, pita, chopped salad and olives as a simple breakfast plate.", prepHe: "מגישים ביצים, קוטג׳, פיתה, סלט קצוץ וזיתים כצלחת בוקר פשוטה." },
  { id: "chicken-shawarma-rice-box", en: "Chicken Shawarma Rice Box", he: "קופסת אורז ושווארמה עוף", slots: ["lunch", "dinner"], diet: "omnivore", items: [["chicken-breast", 170], ["white-rice", 180], ["hummus", 45], ["tomato", 80], ["cucumber", 80]], prepEn: "Use prepared chicken, rice, hummus and chopped vegetables for a practical supermarket box.", prepHe: "מרכיבים קופסה מעשית של עוף מוכן, אורז, חומוס וירקות קצוצים." },
  { id: "tuna-pita-salad-box", en: "Tuna Pita Salad Box", he: "קופסת טונה, פיתה וסלט", slots: ["lunch", "dinner"], diet: "pescatarian", items: [["tuna", 120], ["pita", 60], ["tomato", 100], ["cucumber", 100], ["tahini", 15]], prepEn: "Fill pita with tuna and serve with salad and a small tahini portion.", prepHe: "ממלאים פיתה בטונה ומגישים עם סלט ומנת טחינה קטנה." },
  { id: "labneh-egg-pita", en: "Labneh, Egg & Pita Plate", he: "צלחת לבנה, ביצה ופיתה", slots: ["breakfast", "lunch"], diet: "vegetarian", items: [["labneh", 100], ["eggs", 100], ["pita", 60], ["tomato", 100], ["cucumber", 80], ["herbs", 5]], prepEn: "Serve labneh, boiled eggs, pita, cucumber and tomato with herbs.", prepHe: "מגישים לבנה, ביצים קשות, פיתה, מלפפון ועגבנייה עם עשבי תיבול." },
  { id: "cottage-rice-cakes-vegetables", en: "Cottage Rice Cakes & Vegetables", he: "קוטג׳, פריכיות וירקות", slots: ["snack", "breakfast"], diet: "vegetarian", items: [["cottage-cheese", 180], ["rice-cakes", 36], ["tomato", 100], ["cucumber", 100], ["avocado", 40]], prepEn: "Top rice cakes with cottage cheese and serve with fresh vegetables and avocado.", prepHe: "מורחים קוטג׳ על פריכיות ומגישים עם ירקות טריים ואבוקדו." },
  { id: "ready-chicken-hummus-salad", en: "Ready Chicken Hummus Salad Box", he: "קופסת עוף, חומוס וסלט מוכנה", slots: ["lunch", "dinner"], diet: "omnivore", items: [["chicken-breast", 160], ["hummus", 70], ["pita", 60], ["lettuce", 80], ["tomato", 100], ["cucumber", 80]], prepEn: "Combine prepared chicken, hummus, pita and a ready chopped salad.", prepHe: "משלבים עוף מוכן, חומוס, פיתה וסלט קצוץ מוכן." },
...
function resolveMealImage(mealId) {
  // Some of the original PNG renders are small source previews. Prefer the
  // catalog's 960px WebP plate so the large meal panel never upscales a soft
  // 127px/449px image just because that older file happens to exist first.
...
// Practicality is a property of how a meal can be used, not a diet rule. The
// catalog remains global, while these tags let a person choose supermarket
// ready food, a short assembly, or a normal cooked meal without inventing a
// second country-specific menu. The Israeli/Mediterranean tag is deliberately
// opt-in and only boosts familiar staples; it never overrides restrictions.
const PRACTICAL_MEAL_METADATA = {
  ...Object.fromEntries(expansionMeals.map(meal => [meal.id, meal])),
  ...Object.fromEntries(everydayMeals.map(meal => [meal.id, { format: meal.format, minutes: meal.minutes, styles: ["mediterranean", "israeli"], prepEn: meal.prepEn, prepHe: meal.prepHe }])),
  "cottage-cheese-toast-tomato": { format: "quick", minutes: 5, styles: ["mediterranean", "israeli"], prepEn: "Assemble cottage cheese, bread, tomato and cucumber.", prepHe: "מרכיבים קוטג׳, לחם, עגבנייה ומלפפון." },
  "greek-yogurt-granola-berries": { format: "quick", minutes: 3, styles: ["mediterranean"], prepEn: "Combine high-protein yogurt, granola and berries.", prepHe: "מערבבים יוגורט עשיר בחלבון, גרנולה ופירות יער." },
  "greek-yogurt-fruit-honey": { format: "ready", minutes: 0, styles: ["mediterranean"], prepEn: "Open and serve the yogurt with fruit and honey.", prepHe: "מגישים יוגורט עם פרי ודבש ללא בישול." },
  "greek-yogurt-honey-almonds": { format: "ready", minutes: 0, styles: ["mediterranean"], prepEn: "A ready-to-eat yogurt, honey and almond snack.", prepHe: "חטיף מוכן של יוגורט, דבש ושקדים." },
  "skyr-apple-walnut-bowl": { format: "ready", minutes: 0, styles: ["mediterranean"], prepEn: "A ready-to-eat skyr, apple and walnut snack.", prepHe: "חטיף מוכן של סקיר, תפוח ואגוזי מלך." },
  "skyr-berries-bowl": { format: "ready", minutes: 0, styles: ["mediterranean"], prepEn: "A ready-to-eat skyr and berry snack.", prepHe: "חטיף מוכן של סקיר ופירות יער." },
  "cottage-cheese-fruit-bowl": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "A ready-to-eat cottage cheese and fruit bowl.", prepHe: "קערת קוטג׳ ופירות מוכנה לאכילה." },
  "cornflakes-milk-banana": { format: "ready", minutes: 0, styles: ["israeli"], prepEn: "A ready cereal, milk and banana breakfast.", prepHe: "ארוחת בוקר מוכנה של קורנפלקס, חלב ובננה." },
  "protein-shake-banana": { format: "quick", minutes: 2, styles: ["israeli"], prepEn: "Shake protein powder with milk and take a banana.", prepHe: "מערבבים אבקת חלבון עם חלב ולוקחים בננה." },
  "tuna-rice-cakes": { format: "quick", minutes: 4, styles: ["mediterranean", "israeli"], prepEn: "Open tuna and assemble it with rice cakes and cucumber.", prepHe: "פותחים טונה ומרכיבים עם פריכיות ומלפפון." },
  "hummus-veggie-sticks": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "A supermarket-ready hummus, pita and vegetables plate.", prepHe: "צלחת מוכנה מהסופר של חומוס, פיתה וירקות." },
  "cheese-crackers-grapes": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "A ready-to-eat cheese, rice-cake and grape snack.", prepHe: "חטיף מוכן של גבינה, פריכיות וענבים." },
  "labneh-cucumber-zaatar-toast": { format: "quick", minutes: 5, styles: ["mediterranean", "israeli"], prepEn: "Assemble labneh, cucumber and za'atar on toast.", prepHe: "מרכיבים לבנה, מלפפון וזעתר על לחם קלוי." },
  "salmon-cottage-open-sandwich": { format: "quick", minutes: 5, styles: ["mediterranean", "israeli"], prepEn: "Assemble smoked salmon, cottage cheese and cucumber on bread.", prepHe: "מרכיבים סלמון מעושן, קוטג׳ ומלפפון על לחם." },
  "turkey-club-sandwich": { format: "quick", minutes: 5, styles: ["israeli"], prepEn: "Assemble turkey, salad vegetables and cheese in bread.", prepHe: "מרכיבים הודו, ירקות וגבינה בכריך." },
  "turkey-cucumber-rollups": { format: "quick", minutes: 5, styles: ["israeli"], prepEn: "Roll ready turkey slices with cucumber and cream cheese.", prepHe: "מגלגלים פרוסות הודו מוכנות עם מלפפון וגבינת שמנת." },
  "tuna-pasta-salad": { format: "quick", minutes: 5, styles: ["mediterranean", "israeli"], prepEn: "Use ready pasta, canned tuna and vegetables for a quick salad.", prepHe: "משתמשים בפסטה מוכנה, טונה וירקות לסלט מהיר." },
  "tuna-rice-avocado-bowl": { format: "quick", minutes: 5, styles: ["mediterranean", "israeli"], prepEn: "Combine canned tuna with ready rice, avocado and cucumber.", prepHe: "משלבים טונה עם אורז מוכן, אבוקדו ומלפפון." },
  "chicken-hummus-bowl": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "Choose prepared chicken, hummus, pita and a ready salad.", prepHe: "בוחרים עוף מוכן, חומוס, פיתה וסלט מוכן." },
  "chicken-tzatziki-souvlaki-box": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "A prepared chicken, grain and salad box.", prepHe: "קופסה מוכנה של עוף, דגן וסלט." },
  "chicken-shawarma-pita": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "A prepared chicken shawarma pita with hummus and vegetables.", prepHe: "פיתת שווארמה עוף מוכנה עם חומוס וירקות." },
  "grilled-chicken-rice-broccoli": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "A ready chicken, rice and vegetables combination.", prepHe: "שילוב מוכן של עוף, אורז וירקות." },
  "chicken-caprese-farro-salad": { format: "ready", minutes: 0, styles: ["mediterranean"], prepEn: "A ready chicken, grain and vegetable salad.", prepHe: "סלט מוכן של עוף, דגן וירקות." },
  "tofu-greek-salad-pita": { format: "quick", minutes: 5, styles: ["mediterranean", "israeli"], prepEn: "Assemble ready tofu, pita and Greek-style salad vegetables.", prepHe: "מרכיבים טופו מוכן, פיתה וירקות בסגנון יווני." },
  "crunchy-lupin-bean-salad-cup": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "A ready-to-eat lupin bean and vegetable cup.", prepHe: "כוס מוכנה לאכילה של תורמוס וירקות." },
  "spicy-roasted-broad-beans": { format: "ready", minutes: 0, styles: ["mediterranean", "israeli"], prepEn: "A ready-to-eat broad bean snack.", prepHe: "חטיף מוכן של פול קלוי." }
  ,"israeli-breakfast-plate": { format: "ready", minutes: 3, styles: ["mediterranean", "israeli"], prepEn: "A familiar Israeli breakfast assembled from supermarket staples.", prepHe: "ארוחת בוקר ישראלית מוכרת שמרכיבים ממוצרי סופר." }
  ,"chicken-shawarma-rice-box": { format: "ready", minutes: 2, styles: ["mediterranean", "israeli"], prepEn: "A practical ready chicken, rice, hummus and salad box.", prepHe: "קופסה מעשית של עוף, אורז, חומוס וסלט." }
  ,"tuna-pita-salad-box": { format: "quick", minutes: 4, styles: ["mediterranean", "israeli"], prepEn: "A quick tuna, pita and salad combination.", prepHe: "שילוב מהיר של טונה, פיתה וסלט." }
  ,"labneh-egg-pita": { format: "ready", minutes: 4, styles: ["mediterranean", "israeli"], prepEn: "A simple labneh, egg and pita plate.", prepHe: "צלחת פשוטה של לבנה, ביצה ופיתה." }
  ,"cottage-rice-cakes-vegetables": { format: "ready", minutes: 2, styles: ["israeli"], prepEn: "A no-cook cottage cheese, rice cake and vegetable snack.", prepHe: "נשנוש ללא בישול של קוטג׳, פריכיות וירקות." }
  ,"ready-chicken-hummus-salad": { format: "ready", minutes: 2, styles: ["mediterranean", "israeli"], prepEn: "A ready chicken, hummus, pita and chopped salad box.", prepHe: "קופסה מוכנה של עוף, חומוס, פיתה וסלט קצוץ." }
};

const PRACTICALITY_DEFAULT = { format: "cook", minutes: 25, styles: [] };

// Curated everyday combinations, shared by generated plans and manual discovery.
// Restrict the style before selection so AI and deterministic generation agree.
const SUPERMARKET_MEALS = new Set([
  ...expansionMeals.map(meal => meal.id),
  ...everydayMeals.map(meal => meal.id),
  "cottage-cheese-toast-tomato", "greek-yogurt-granola-berries", "greek-yogurt-fruit-honey",
  "scrambled-eggs-toast-avocado", "veggie-omelette-toast", "cornflakes-milk-banana",
  "protein-shake-banana", "tuna-rice-cakes", "hummus-veggie-sticks", "cheese-crackers-grapes",
  "turkey-club-sandwich", "turkey-cucumber-rollups", "tuna-pasta-salad", "tuna-rice-avocado-bowl",
  "chicken-shawarma-pita", "grilled-chicken-rice-broccoli", "tofu-greek-salad-pita",
  "israeli-breakfast-plate", "chicken-shawarma-rice-box", "tuna-pita-salad-box", "labneh-egg-pita", "cottage-rice-cakes-vegetables", "ready-chicken-hummus-salad",
  "baked-potato-cottage-cheese", "cottage-cheese-fruit-bowl", "greek-yogurt-honey-almonds",
  "skyr-apple-walnut-bowl", "skyr-berries-bowl", "rice-cakes-peanut-butter-banana",
  "lentil-stew-rice", "egg-fried-rice-vegetables", "chicken-potato-carrot-tray"
]);
...
const CATALOG = MEALS.map(meal => {
  ...
  const practical = PRACTICAL_MEAL_METADATA[meal.id] || PRACTICALITY_DEFAULT;
  return {
    ...meal,
    allergens,
    nutrients,
    ...
    mealFormat: practical.format,
    prepMinutes: practical.minutes,
    foodStyles: [...practical.styles, ...(SUPERMARKET_MEALS.has(meal.id) ? ["supermarket"] : [])],
    prepEn: practical.prepEn || meal.prepEn,
    prepHe: practical.prepHe || meal.prepHe
  };
});
...
const CATEGORY_DEFINITIONS = {
  supermarket: { labelEn: "Everyday / supermarket", labelHe: "יומיומי / מהסופר", rule: meal => meal.foodStyles.includes("supermarket") },
...
const PREFERENCE_NOISE = /\b(no|none|without|avoid|avoiding|dislike|don't|dont|not|preference|please|foods?|food|אין|ללא|בלי|להימנע|לא|אוהב|רוצה|מזון)\b/giu;

function parseFoodPreferenceTerms(...texts) {
  const terms = texts
    .filter(Boolean)
    .flatMap((text) => String(text).toLocaleLowerCase().split(/[,;|\n/]+/u))
    .flatMap((part) => {
      const cleaned = part.replace(PREFERENCE_NOISE, " ").replace(/\s+/g, " ").trim();
      return [cleaned, ...cleaned.split(/\s+/u)];
    })
    .map((term) => term.trim())
    .filter((term) => term.length >= 3)
    .filter((term) => !/^(none|no preference|אין העדפה|ללא הערות)$/iu.test(term));
  return [...new Set(terms)];
}

function mealMatchesPreferenceTerms(meal, terms = []) {
  if (!Array.isArray(terms) || !terms.length) return false;
  const haystack = (meal.aliases || []).map((value) => String(value).toLocaleLowerCase());
  return terms.some((term) => haystack.some((candidate) => candidate.includes(term)));
}
...
function filterMeals({
  ...
  const allowedFormats = mealFormatPreference === "ready"
    ? new Set(["ready"])
    : mealFormatPreference === "quick"
      ? new Set(["ready", "quick"])
      : mealFormatPreference === "cook"
        ? new Set(["cook"])
        : null;
  const maximumMinutes = prepTimePreference === "zero" ? 0
    : prepTimePreference === "five" ? 5
      : prepTimePreference === "fifteen" ? 15
        : Infinity;
  return CATALOG.filter(meal => {
    if (foodStylePreference === "supermarket" && !meal.foodStyles.includes("supermarket")) return false;
    ...
    if (allowedFormats && !allowedFormats.has(meal.mealFormat)) return false;
    if (meal.prepMinutes > maximumMinutes) return false;
    if (mealMatchesPreferenceTerms(meal, avoidTerms)) return false;
    return true;
  });
}
...
function catalogForPrompt(meals, isHebrew = false) {
  return meals
    .map(meal => {
      const tags = [meal.diet, meal.mealFormat, `${meal.prepMinutes}m`, ...meal.foodStyles, ...meal.nutrients].join(",");
...
function selectMeals({
  ...
  return pool
    ...
    .map(meal => {
      ...
      // A diagnosed nutrient concern is a selection constraint, not merely a
      // tiny tie-breaker. Keep calorie/macro fit relevant, but make sure a
      // matching catalog option is surfaced whenever that slot has one.
      const nutrientPriority = preferNutrients.length && nutrientHits === 0 ? 1 : 0;
      const favoriteHits = mealMatchesPreferenceTerms(meal, favoriteTerms) ? 1 : 0;
      const formatBoost = mealFormatPreference === "mix" ? 0
        : mealFormatPreference === "ready" && meal.mealFormat === "ready" ? 0.8
          : mealFormatPreference === "quick" && meal.mealFormat !== "cook" ? 0.6
            : mealFormatPreference === "cook" && meal.mealFormat === "cook" ? 0.45
              : 0;
      const localeBoost = foodStylePreference === "mix" ? 0
        : meal.foodStyles.includes(foodStylePreference) ? 0.6
          : 0;
      const calorieWeight = macroAware ? 1.5 : 1;
      return { meal, score: nutrientPriority * 100 + calorieGap * calorieWeight + macroScore - nutrientHits * 0.25 - favoriteHits * 0.25 - formatBoost - localeBoost };
    })
