...
const DEFAULT_OPENAI_WORKOUT_MODEL = "gpt-4.1-mini";
...
function logOpenAiStartupDiagnostics(env = process.env, log = console.log) {
  ...
  log(
    ...
    JSON.stringify({
      ...
      selectedModel: env.OPENAI_CHAT_MODEL || DEFAULT_OPENAI_CHAT_MODEL,
      selectedWorkoutModel: env.OPENAI_WORKOUT_MODEL || env.OPENAI_CHAT_MODEL || DEFAULT_OPENAI_WORKOUT_MODEL,
...
const INCOMPLETE_RESPONSE_MESSAGES = {
  en: "The workout service returned an incomplete response. Please try again.",
  he: "שירות יצירת האימון החזיר תשובה חלקית. נסה שוב."
};

function incompleteResponseMessage(language) {
  return language === "he" ? INCOMPLETE_RESPONSE_MESSAGES.he : INCOMPLETE_RESPONSE_MESSAGES.en;
}
