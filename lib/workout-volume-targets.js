...
// In a calorie deficit, the program prioritizes high-quality work and
// recovery.  This is deliberately an additional ceiling compression rather
// than a claim that muscle gain is impossible while dieting: some athletes
// can still gain muscle, but the default should not prescribe mass-phase
// volume.  At 4+ days for an intermediate athlete, chest therefore caps at
// 13 effective weekly sets instead of the general-fitness ceiling of 17.
const FAT_LOSS_VOLUME_MULTIPLIER = Object.freeze({ min: 1, max: 0.75 });
...
function resolveMultipliers(profile) {
  ...
  const fatLoss = isFatLossGoal(profile.goal)
    ? FAT_LOSS_VOLUME_MULTIPLIER
    : { min: 1, max: 1 };
  return {
    min: experience.min * goalProfile.min * daysMultiplier.min * fatLoss.min,
    max: experience.max * goalProfile.max * daysMultiplier.max * fatLoss.max
  };
}
...
function preferredRangeForMuscle(muscle, profile = {}) {
  ...
  const isAdvanced = ["advanced", "professional"].includes(String(profile.experience || "").trim().toLowerCase());
