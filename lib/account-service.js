...
const USER_SUBCOLLECTIONS = Object.freeze([
  "athleteCore", "settings", "workoutPlans", "nutritionPlans", "dailyNutritionLogs",
  "customFoods", "savedFoodCombinations", "workoutLogs",
...
class AccountService {
  ...
  async exportAccount(uid) {
    ...
    for (const conversation of conversationSnapshot.docs) {
      ...
      for (const item of messages.docs) {
        const message = item.data() || {};
        if (message.type !== "music_link" || message.senderUid !== uid) continue;
        musicLinks.push({
          id: item.id,
          conversationId: conversation.id,
          provider: String(message.music?.provider || "link"),
          title: String(message.music?.title || ""),
          url: message.deletedAt || message.music?.unavailable ? "" : String(message.music?.url || ""),
          available: !message.deletedAt && message.music?.unavailable !== true,
          createdAt: serializable(message.createdAt)
        });
      }
    }
  ...
  async scrubOwnedMusicLinks(uid, conversations) {
    let scrubbed = 0;
    for (const conversation of conversations) {
      const messages = await conversation.ref.collection("messages").get();
      for (const item of messages.docs) {
        const message = item.data() || {};
        if (message.type !== "music_link" || message.senderUid !== uid || message.deletedAt || message.music?.unavailable) continue;
        await item.ref.set({
          music: {
            provider: String(message.music?.provider || "link"),
            title: "",
  ...
  async deleteAccount(uid, { confirmed = false, reauthenticatedAt = 0 } = {}) {
    ...
    try {
      if (!cleanupReady) {
        const media = {
          images: await this.cleanupImageReferences(uid),
          voice: { deleted: 0, unavailable: 0, ownershipMismatch: 0 },
          music: { scrubbed: 0 }
        };
        ...
        media.music = await this.scrubOwnedMusicLinks(uid, conversationSnapshot.docs);
        for (const conversation of conversationSnapshot.docs) {
