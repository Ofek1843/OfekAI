...
function createTelemetryAgent({
  ...
  const state = {
    ...
    uniqueVisitorsSinceStart: new Set(),
    registrationsByDay: new Map(),
    verifiedRegistrationHashes: new Set(),
  ...
  function buildHighestRoiAction({
    visitorsToday,
    registrationsToday,
    workoutPlansSavedToday,
    nutritionPlansSavedToday,
    errorRequests,
    stats,
    topEvents
  }) {
    const pricingInterest = Number(topEvents.get("pricing_page_view") || 0)
      + Number(topEvents.get("pricing_click") || 0)
      + Number(topEvents.get("upgrade_clicked") || 0);

    if (errorRequests > 0) {
      return "לטפל קודם בשגיאות ה-API של היום — כל תקלה חיה עלולה לפגוע בהרשמות, שמירות ואמון.";
    }

    if (visitorsToday >= 10 && registrationsToday === 0) {
      return "יש תנועה לאתר אבל אין הרשמות היום — כדאי לבדוק קודם את ה-CTA הראשי, את טופס ההרשמה ואת מסרי האמון בעמוד הנחיתה.";
    }

    if (pricingInterest >= 2 && Number(stats?.activeProSubscribers || 0) === 0) {
      return "יש עניין במסלול פרו אבל עדיין אין מנויים פעילים — כדאי לבדוק קודם את ניסוח ההצעה, המחיר והחיכוך בדרך לשדרוג.";
    }

    if (visitorsToday > 0 && workoutPlansSavedToday === 0 && nutritionPlansSavedToday === 0) {
      return "יש משתמשים שנכנסו אבל לא נשמרו היום תוכניות או תפריטים — כדאי לבדוק קודם אם הדרך מהדשבורד למחוללים מספיק ברורה ומשכנעת.";
    }

    if (visitorsToday < 5) {
      return "התנועה היום עדיין נמוכה — ה-ROI הגבוה ביותר כרגע הוא להביא עוד תנועה איכותית לפני שמוסיפים עוד פיצ'רים.";
    }

    return "כרגע אין תקלה בולטת או דליפת המרה חריגה — כדאי להתמקד בהבאת עוד תנועה איכותית לעמוד הנחיתה ולבדוק איך היא מתנהגת.";
  }

  function buildDailyDigest(stats) {
    const todayKey = formatIsoDate();
    const visitorsToday = Number(state.uniqueVisitorsByDay.get(todayKey)?.size || 0);
    const visitorsSinceStart = Number(state.uniqueVisitorsSinceStart.size || 0);
    ...
    const totalRegisteredUsers = Number(stats?.registeredUsers || 0);
    const totalSavedWorkoutPlans = Number(stats?.savedWorkoutPlans || 0);
    const totalSavedNutritionPlans = Number(stats?.savedNutritionPlans || 0);
    ...
    const topEventsMap = new Map(state.analyticsEvents.entries());
    ...
    const conversionToday = visitorsToday > 0
      ? `${((registrationsToday / visitorsToday) * 100).toFixed(1)}%`
      : "0%";

    const highestRoiAction = buildHighestRoiAction({
      visitorsToday,
      registrationsToday,
      workoutPlansSavedToday,
      nutritionPlansSavedToday,
    ...
    return [
      ...
      `<b>מבקרים ייחודיים היום</b>: ${visitorsToday.toLocaleString("he-IL")}`,
      `<b>מבקרים ייחודיים מאז שהסוכן הופעל</b>: ${visitorsSinceStart.toLocaleString("he-IL")}`,
      `<b>סה״כ נרשמו לאתר</b>: ${totalRegisteredUsers.toLocaleString("he-IL")}`,
      `<b>נרשמים חדשים היום</b>: ${registrationsToday.toLocaleString("he-IL")}`,
      `<b>יחס הרשמה היום</b>: ${conversionToday}`,
      `<b>סה״כ תוכניות אימון שמורות</b>: ${totalSavedWorkoutPlans.toLocaleString("he-IL")}`,
      `<b>תוכניות אימון שנשמרו היום</b>: ${workoutPlansSavedToday.toLocaleString("he-IL")}`,
      `<b>סה״כ תפריטי תזונה שמורים</b>: ${totalSavedNutritionPlans.toLocaleString("he-IL")}`,
      ...
      recentErrors,
      "",
      `הפעולה עם ה-ROI הגבוה ביותר שכדאי לעשות עכשיו: ${highestRoiAction}`
    ].join("\n");
  }
  ...
  function recordAnalytics(eventName, context = {}) {
    ...
    // Browser analytics is intentionally unauthenticated and can be replayed.
    // It is useful for funnel analysis, but never authoritative enough to
    // trigger an operator notification or count a verified registration.
    ...
    if ((event === "page_view" || event === "landing_page_view") && context.visitorToken) {
      // The application does not submit IP address or raw user-agent data to
      // telemetry. An optional opaque, client-generated token is the only
      // supported coarse visitor-deduplication input.
      const dailyFingerprint = shortHash([date, context.visitorToken].join("|"));
      const overallFingerprint = shortHash(context.visitorToken);
      state.uniqueVisitorsByDay.set(date, state.uniqueVisitorsByDay.get(date) || new Set());
      state.uniqueVisitorsByDay.get(date).add(dailyFingerprint);
      state.uniqueVisitorsSinceStart.add(overallFingerprint);
    }
  }

  async function recordVerifiedRegistration({ uid } = {}) {
    const registrationHash = shortHash(safeText(uid, 180));
    if (!uid || state.verifiedRegistrationHashes.has(registrationHash)) return false;
    state.verifiedRegistrationHashes.add(registrationHash);
    // Bound the in-memory replay guard without retaining raw user identifiers.
    if (state.verifiedRegistrationHashes.size > 2_000) {
      state.verifiedRegistrationHashes = new Set([...state.verifiedRegistrationHashes].slice(-1_000));
    }

    const date = formatIsoDate();
    incrementMap(state.registrationsByDay, date);
    const registrationsToday = Number(state.registrationsByDay.get(date) || 0);
    return sendTelegramMessage([
      `🎉 <b>${escapeHtml(brandName)} — משתמש חדש נרשם</b>`,
      `שעה: ${escapeHtml(formatClock())} (שעון ישראל)`,
      `נרשמים חדשים היום: ${registrationsToday.toLocaleString("he-IL")}`,
      "ההתראה אינה כוללת שם, אימייל או מזהה משתמש."
    ].join("\n"));
  }
