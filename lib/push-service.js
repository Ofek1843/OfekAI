...
class PushNotificationService {
  ...
  async notifySocialMessage({ senderUid, conversationId, messageId }) {
    ...
    if (message.type === "text") {
      ...
      type = "voice_message";
    } else if (message.type === "music_link") {
      if (preferences.newMessages === false) return { skipped: "messages_disabled" };
      // Music destinations and titles stay out of the push payload. The
      // authenticated conversation resolves the safe link after open.
      type = "message";
      text = "";
