...
class PushNotificationService {
  ...
  async sendWorkoutCheckin({ uid, checkinId, planId, workoutId, sessionName, scheduledDate, promptNumber, locale }) {
    const type = "workout_checkin";
    const eventId = `workout-checkin:${uid}:${checkinId}:${Number(promptNumber) || 1}`;
    const copy = notificationCopy({ type, locale, sessionName, eventSeed: eventId });
    return this.sendEvent({
      eventId,
      recipientUid: uid,
      ...
      title: copy.title,
      body: copy.body,
      url: `/dashboard.html?checkin=${encodeURIComponent(checkinId)}&source=notification&plan=${encodeURIComponent(planId)}&workout=${encodeURIComponent(workoutId)}&date=${encodeURIComponent(scheduledDate)}`
    });
  }
