...
class FirestorePushStore {
  ...
  async listReminderPreferences(limit = 500) {
    const snapshot = await this.db.collection("notificationPreferences").where("eligibleForWorkoutReminders", "==", true).limit(limit).get();
    return snapshot.docs.map((item) => ({ uid: item.id, ...normalizePreferences(item.data()) }));
  }
