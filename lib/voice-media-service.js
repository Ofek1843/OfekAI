...
class VoiceMediaService {
  ...
  async playbackUrl({ uid, conversationId, assetId }) {
    ...
    const url = typeof this.provider.getPlaybackUrl === "function"
      ? this.provider.getPlaybackUrl({ assetId, source, ttlSeconds: this.config.playbackTtlSeconds })
      : this.provider.getSignedUrl(source, this.config.playbackTtlSeconds);
    const isHttpsPlayback = /^https:\/\//i.test(String(url || ""));
    const isGuardedLocalPlayback = this.provider.localReview === true
      && /^http:\/\/(127\.0\.0\.1|localhost)(:\d+)?\/api\/local-review\/voice\/local_[a-f0-9]{32}\?/i.test(String(url || ""));
    if (!isHttpsPlayback && !isGuardedLocalPlayback) {
      throw providerError("voice_playback_failed", "The voice message could not be prepared for playback.");
    }
