...
// This is deliberately a separate, narrow public contract. Operational
// telemetry still needs richer internal aggregates, but a landing-page visitor
// must never receive revenue, subscription, nutrition, logging, or fallback
// diagnostic data just to render two aggregate social-proof counters.
function toPublicSocialProof(stats = {}) {
  const wholeNumber = (value) => Math.max(0, Math.floor(Number(value) || 0));
  return {
    registeredUsers: wholeNumber(stats.registeredUsers),
    savedWorkoutPlans: wholeNumber(stats.savedWorkoutPlans)
  };
}
