...
const { allTargetRanges, volumeStatus } = require("./workout-volume-targets");
const {
  getCatalogExercise,
  getDisabledExercise,
...
function repairExercisesMissingFromCatalog(program, context, repairs) {
  ...
  for (let i = 0; i < program.sessions.length; i++) {
    ...
    session.exercises = session.exercises.filter((exercise) => {
      ...
      const primaryMuscle = normalizeMuscleGroup(exercise.muscleGroup);
      // With no usable muscle group there is nothing to match a replacement
      // against, and deleting an exercise purely because its metadata is
      // incomplete would discard content the user asked for. Leave it for
      // validation to judge.
      if (!primaryMuscle) return true;
...
// lib/workout-volume-targets.js) by adjusting SETS ONLY on exercises that
// already exist in the program — never inflates sets blindly, never adds or
// removes exercises, never touches equipment, and never pushes a session
// over its duration budget. This is a bounded, best-effort nudge, not a
// guarantee every muscle lands in range: a program with genuinely too few
// exercises for a muscle, or already at the session-duration ceiling, is
// left for the caller to judge via the returned volume/status, not silently
// forced into range.
//
// Only PRIMARY movers (setCredits credit === 1.0 for the muscle) are
// adjusted, so nudging one muscle's volume doesn't meaningfully perturb
// others via their fractional/indirect credit on the same exercise.
const MAX_VOLUME_REPAIR_PASSES = 2;
const MAX_SETS_PER_EXERCISE = 6;
const MIN_SETS_PER_EXERCISE = 2;

function repairWeeklyVolumeTargets(program, context, repairs) {
  // Opt-in, not opt-out: this pass changes SET COUNTS, which every other
  // repair pass in this file deliberately never does (they substitute or
  // remove exercises, never inflate/deflate their sets). Callers that don't
  // opt in — e.g. isolated single-exercise repairs, or tests exercising a
  // specific earlier pass — must not have their fixtures silently perturbed
  // by a volume nudge they never asked for. server.js opts in explicitly for
  // the real /api/workout-builder generation path only; the reroll
  // endpoint's synthetic single-exercise repair never opts in.
    ...
    for (const [muscle, range] of Object.entries(targets)) {
      ...
      for (let s = 0; s < program.sessions.length; s++) {
        ...
        for (const exercise of session.exercises) {
          ...
          if (credits && credits[muscle] === 1) {
            candidates.push({ session, exercise, sessionIndex: s });
          }
        }
      }
      ...
      if (status === "below") {
        ...
        if (estimate.estimatedMinutes > budget) {
          target.exercise.sets = currentSets; // would exceed the duration budget — revert
          continue;
        }
        repairs.push(
        ...
        const currentSets = Number(target.exercise.sets) || 0;
        if (currentSets <= MIN_SETS_PER_EXERCISE) continue;
