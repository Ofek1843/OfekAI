...
const { buildVolumeLedger, buildQualityDiagnostic, targetPointForPolicy } = require("./workout-volume-ledger");
...
const { isFatLossGoal } = require("./workout-priority");
...
function volumeProfile(program, context = {}) {
  return {
    goal: context.goal || program?.goal,
...
function isTimedPrescription(reps) {
  return /(?:\bsec(?:ond)?s?\b|שנ(?:יות)?|דק(?:ות)?)/i.test(String(reps || ""));
}

function isCompoundExercise(exercise) {
  const credits = EXERCISE_SETCREDITS[exercise?.exerciseId] || {};
  return Object.values(credits).filter((credit) => Number(credit) > 0).length > 1;
}

function cuttingRepRange(exercise) {
  const raw = String(exercise?.reps || "").trim();
  if (!raw || isTimedPrescription(raw)) return raw;
  const match = raw.match(/^(\d+)\s*-\s*(\d+)$/);
  if (!match) {
    const single = Number(raw);
    if (!Number.isFinite(single) || single >= 8) return raw;
    return isCompoundExercise(exercise) ? "8-12" : "10-15";
  }
  const lower = Number(match[1]);
  const upper = Number(match[2]);
  if (lower >= 8 && upper <= 30) return `${lower}-${upper}`;
  return isCompoundExercise(exercise) ? "8-12" : "10-15";
}

// A cut needs enough load to retain strength and muscle, but it should not
// inherit long strength-phase rests or low-rep fatigue from a mass phase.
// This is a deterministic safety backstop for both provider and fallback
// programs; it operates alongside the lower fat-loss volume ceiling in
// workout-volume-targets.js.
function applyFatLossProgramming(program, context, repairs) {
  if (!isFatLossGoal(context.goal || program?.goal)) return;
  for (let sessionIndex = 0; sessionIndex < program.sessions.length; sessionIndex += 1) {
    for (const exercise of program.sessions[sessionIndex]?.exercises || []) {
      const compound = isCompoundExercise(exercise);
      const minimumRest = compound ? 75 : 45;
      const maximumRest = compound ? 90 : 75;
      const oldRest = Number(exercise.restSeconds);
      const nextRest = Math.max(minimumRest, Math.min(maximumRest, Number.isFinite(oldRest) ? oldRest : maximumRest));
      const nextReps = cuttingRepRange(exercise);

      if (exercise.restSeconds !== nextRest) {
        repairs.push(`Session ${sessionIndex + 1}: adjusted "${exercise.name}" rest to ${nextRest}s for the fat-loss recovery profile.`);
        exercise.restSeconds = nextRest;
      }
      if (nextReps && exercise.reps !== nextReps) {
        repairs.push(`Session ${sessionIndex + 1}: adjusted "${exercise.name}" to ${nextReps} reps for the fat-loss recovery profile.`);
        exercise.reps = nextReps;
      }
    }
  }
}
...
function repairSchemaDefects(program, repairs) {
  for (let i = 0; i < program.sessions.length; i++) {
    ...
    for (const exercise of session.exercises) {
      ...
      // A working-set prescription above four is neither needed for this
      // product's hypertrophy programming model nor safe to carry forward
      // into later repair passes.  Keep the cap here as a backstop for
      // provider-generated plans as well as local plans.
      if (Number.isInteger(exercise.sets) && exercise.sets > MAX_SETS_PER_EXERCISE) {
        const previousSets = exercise.sets;
        exercise.sets = MAX_SETS_PER_EXERCISE;
        repairs.push(`Session ${i + 1}: capped "${exercise.name}" from ${previousSets} to ${MAX_SETS_PER_EXERCISE} working sets.`);
      }
  ...
  for (let step = 0; step < MAX_ISOLATION_TRIM_STEPS; step += 1) {
    ...
    for (const entry of muscles) {
      ...
      for (const source of candidates) {
        ...
        const minimumAfterChange = needsHardTrim ? entry.hardMinimum : entry.targetPoint;
...
function swapCompoundVariationForPreferredBalance(program, context, repairs) {
  if (!context.applyVolumeTargets) return;
  const profile = volumeProfile(program, context);
  const selectedEquipmentNorm = new Set(
    (Array.isArray(context.equipment) ? context.equipment : []).map(normalizeEquipment).filter(Boolean)
  );
  const MAX_VARIATION_SWAPS = 12;

  for (let step = 0; step < MAX_VARIATION_SWAPS; step += 1) {
    const ledger = buildVolumeLedger(program, profile);
    const baselineDiagnostic = buildQualityDiagnostic(program, profile);
    const excessMuscles = Object.values(ledger.muscles)
      .filter((entry) => entry.requirement === "required" && entry.amountAbovePreferred > 0)
      .sort((left, right) => right.amountAbovePreferred - left.amountAbovePreferred);
    let best = null;

    for (const excessEntry of excessMuscles) {
      for (const source of excessEntry.sourceExercises) {
        const session = program.sessions[source.sessionIndex];
        const exercise = session?.exercises?.[source.exerciseIndex];
        if (!exercise || exercise.exerciseId !== source.exerciseId) continue;
        const oldCredits = EXERCISE_SETCREDITS[source.exerciseId] || {};
        if (Object.keys(oldCredits).length <= 1) continue;
        const primaryMuscle = primaryMuscleForExerciseId(source.exerciseId);
        if (!primaryMuscle) continue;
        const samePrimaryCompounds = program.sessions.flatMap((item) => item.exercises || [])
          .filter((item) => primaryMuscleForExerciseId(item.exerciseId) === primaryMuscle
            && Object.keys(EXERCISE_SETCREDITS[item.exerciseId] || {}).length > 1).length;
        const usedIds = new Set((session.exercises || []).map((item) => item.exerciseId));

        for (const candidateId of CATALOG_IDS_BY_PRIMARY_MUSCLE.get(primaryMuscle) || []) {
          if (usedIds.has(candidateId) || !isPublicExerciseEnabled(candidateId)) continue;
          const candidateCatalog = getCatalogExercise(candidateId);
          if (!candidateCatalog || !isExerciseEquipmentAllowed(candidateCatalog.equipment, selectedEquipmentNorm)) continue;
          const replacement = replacementFromExerciseId(candidateId);
          if (!replacement || usedIds.has(replacement.exerciseId) || replacement.exerciseId === source.exerciseId) continue;
          const newCredits = EXERCISE_SETCREDITS[replacement.exerciseId] || {};
          if ((Number(newCredits[excessEntry.muscle]) || 0) >= (Number(oldCredits[excessEntry.muscle]) || 0)) continue;
          if (Object.keys(newCredits).length <= 1 && samePrimaryCompounds <= 1) continue;

          const sets = Number(exercise.sets) || 0;
          const affectedMuscles = new Set([...Object.keys(oldCredits), ...Object.keys(newCredits)]);
          let hardSafe = true;
          for (const muscle of affectedMuscles) {
            const entry = ledger.muscles[muscle];
            if (!entry || entry.requirement !== "required") continue;
            const nextTotal = entry.effectiveTotal
              - (Number(oldCredits[muscle]) || 0) * sets
              + (Number(newCredits[muscle]) || 0) * sets;
            if (nextTotal < entry.hardMinimum || nextTotal > entry.hardMaximum) {
              hardSafe = false;
              break;
            }
          }
          if (!hardSafe) continue;

          const trial = JSON.parse(JSON.stringify(program));
          const trialExercise = trial.sessions[source.sessionIndex].exercises[source.exerciseIndex];
          applyReplacementFields(trialExercise, replacement);
          const trialDiagnostic = buildQualityDiagnostic(trial, profile);
          if (trialDiagnostic.score <= baselineDiagnostic.score) continue;
          if (!best || trialDiagnostic.score > best.score) {
            best = { source, replacement, score: trialDiagnostic.score, excessMuscle: excessEntry.muscle };
          }
        }
      }
    }
    ...
    if (!best) break;
    const target = program.sessions[best.source.sessionIndex].exercises[best.source.exerciseIndex];
    const oldName = target.name || target.exerciseId;
    applyReplacementFields(target, best.replacement);
    repairs.push(`Session ${best.source.sessionIndex + 1}: swapped "${oldName}" for "${best.replacement.name}" to reduce ${best.excessMuscle} preferred-range excess while preserving hard coverage.`);
  }
}
...
function repairWeeklyVolumeTargets(program, context, repairs) {
  ...
  function tryAddPrimaryExercise(muscle, range, guard, requestedSets = 3) {
  ...
  for (let pass = 0; pass < MAX_QUALITY_PASSES; pass++) {
    ...
    for (const [muscle, policy] of orderedTargetEntries(policies, profile)) {
      ...
      const directSetsToAdd = Math.max(
        MIN_SETS_PER_EXERCISE,
        Math.min(3, Math.ceil(targetPoint - actual))
      );
      if (!improved && actual + directSetsToAdd <= policy.preferredMax) {
        improved = tryAddPrimaryExercise(
          muscle,
          { min: policy.preferredMin, max: policy.preferredMax },
          (candidateId) => !wouldExceedAnyHardMaximum(candidateId, directSetsToAdd, perMuscle, (m) => policies[m]?.hardMaximum)
            && !wouldExceedAnyHardMaximum(candidateId, directSetsToAdd, perMuscle, (m) => policies[m]?.preferredMax),
          directSetsToAdd
        );
      }
...
function repairWorkoutProgram(program, context = {}) {
  ...
  repairExperienceMismatch(program, context, repairs);
  assignExerciseIds(program, context, repairs);
  repairSchemaDefects(program, repairs);
  applyFatLossProgramming(program, context, repairs);
  ...
  swapCompoundVariationForPreferredBalance(program, context, repairs);
  repairExperienceMismatch(program, context, repairs);

  return { program, repairs };
}

function repairExperienceMismatch(program, context, repairs) {
  if (!context.experience) return;
  const allowed = new Set((context.equipment || []).map(normalizeEquipment));
  for (const session of program.sessions) {
    const used = new Set([...(context.reservedExerciseIds || []), ...(session.exercises || []).map(exercise => exercise.exerciseId)]);
    for (const exercise of session.exercises || []) {
      if (isExerciseLevelSuitable(exercise.exerciseId, context.experience)) continue;
      const primary = primaryMuscleForExerciseId(exercise.exerciseId);
      const candidate = (CATALOG_IDS_BY_PRIMARY_MUSCLE.get(primary) || []).find(id => {
        const entry = getCatalogExercise(id);
        return !used.has(id) && isPublicExerciseEnabled(id)
          && isExerciseLevelSuitable(id, context.experience)
          && isExerciseEquipmentAllowed(entry.equipment, allowed);
      });
      if (candidate) {
        const previous = exercise.exerciseId;
        applyReplacementFields(exercise, replacementFromExerciseId(candidate));
        used.delete(previous);
        used.add(candidate);
        repairs.push(`Replaced ${previous} with ${candidate} for ${context.experience} experience.`);
      }
    }
  }
}

module.exports = { repairWorkoutProgram, resolveExerciseId, repairWeeklyVolumeTargets, diagnoseVolumeGateFailure, applyFatLossProgramming };
