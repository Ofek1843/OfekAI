...
function repairWeeklyVolumeTargets(program, context, repairs) {
  ...
  function tryAddPrimaryExercise(muscle, range, guard) {
    ...
    for (let s = 0; s < program.sessions.length; s++) {
      ...
      const replacementId = primaryCandidateIds.find((candidateId) => {
        ...
        if (!isPublicExerciseEnabled(candidateId)) return false;
        const candidate = getCatalogExercise(candidateId);
        if (!candidate || !isExerciseEquipmentAllowed(candidate.equipment, selectedEquipmentNorm)) return false;
        return !guard || guard(candidateId);
      });
  ...
  for (let pass = 0; pass < MAX_QUALITY_PASSES; pass++) {
    ...
    for (const [muscle, policy] of Object.entries(policies)) {
      ...
      if (actual >= policy.preferredMin || actual >= policy.hardMaximum) continue;
      ...
      for (const target of candidates) {
        ...
        // for this profile.
        if (actual + target.credit > policy.hardMaximum) continue;
      ...
      if (!improved && actual + 3 <= policy.hardMaximum) {
      ...
      if (improved) changedThisPass = true;
    }
