...
function assignExerciseIds(program, context, repairs) {
  ...
  for (let i = 0; i < program.sessions.length; i++) {
    ...
    for (const exercise of session.exercises) {
      ...
      const canonicalMuscleGroup = getCatalogExercise(uniqueId)?.muscleGroup;
      if (canonicalMuscleGroup && exercise.muscleGroup !== canonicalMuscleGroup) {
        exercise.muscleGroup = canonicalMuscleGroup;
        repairs.push(`Session ${i + 1}: corrected "${exercise.name || uniqueId}" primary muscle to ${canonicalMuscleGroup} from the exercise catalog.`);
      }
    }
  }
}
...
function replacementFromExerciseId(exerciseId) {
  ...
  return {
    ...
    muscleGroup: catalogEntry.muscleGroup || Object.keys(catalogEntry.setCredits || {})[0] || "General",
