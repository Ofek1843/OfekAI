...
// 10. working-set cap: 1-4 working sets per exercise per session
// 11. schema: required fields present and types valid, including exerciseId
...
const CANONICAL_EQUIPMENT_ALIASES = {
  ...
  "parallel bars": "parallelbar",
  parallelbars: "parallelbar",
  dipbars: "parallelbar",
  "dip bars": "parallelbar",
  // Hebrew forms the workout-generation prompt explicitly instructs the
  // model to use when Hebrew is selected (see server.js's Hebrew "Equipment:"
  // translation table). Equipment identity must not depend on which
  // language the model happened to write it in — a Hebrew "מכונה" for a
  // user who selected Machines is valid equipment, not missing equipment.
  "משקל גוף": "bodyweight",
  מתח: "pullupbar",
  מכונה: "machine",
  מכונות: "machine",
...
function validateWorkoutProgram(program, context = {}) {
  ...
  const errors = [];
  const warnings = [];
  // Tracked separately from `errors` so the API can expose
  // validationSummary.equipmentPassed without also exposing the raw
  // violation text (that stays in server-side logs only — see server.js).
  ...
  for (let i = 0; i < program.sessions.length; i++) {
    ...
    for (const exercise of session.exercises) {
      ...
      if (!isExerciseLevelSuitable(exercise.exerciseId, context.experience)) {
        errors.push(`Session ${i + 1}: ${exercise.name || exercise.exerciseId} exceeds the selected experience level.`);
      }

      // Bodyweight is equipment like any other canonical token: it passes
      // only when it is a member of the caller-supplied `equipment` set
      // (context.equipment is expected to already be the FINAL allowed
      // canonical set from deriveAllowedEquipment() — see
      // lib/workout-equipment-policy.js — which explicitly includes
      // "bodyweight" for styles where it's intrinsic, e.g. Calisthenics).
      // There is no special case here: an unrecognized or empty equipment
      // value never silently passes, and neither does an equipment value
      // that simply wasn't selected.
      if (selectedNorm.size > 0 && norm !== "" && !selectedNorm.has(norm)) {
        const message = `${exercise.name || "Exercise"} (Session ${i + 1}) requires "${exercise.equipment}", which is not selected.`;
        errors.push(message);
        equipmentErrors.push(message);
      } else if (selectedNorm.size > 0 && norm === "") {
        const message = `${exercise.name || "Exercise"} (Session ${i + 1}) has no recognizable equipment value.`;
        errors.push(message);
        equipmentErrors.push(message);
      }
    }
  ...
  for (let i = 0; i < program.sessions.length; i++) {
    ...
    for (const exercise of session.exercises || []) {
      ...
      if (typeof exercise.sets !== "number" || exercise.sets < 1 || exercise.sets > 4) {
        errors.push(`Session ${i + 1}: working sets must be between 1 and 4.`);
      }
  ...
  const equipmentOk = equipmentErrors.length === 0;
  return { ok, errors, warnings, equipmentOk, equipmentErrors };
}
