...
function callbackEventId(body) {
  const providerId = findValue(body, ["transaction_uid", "transactionUid", "payment_request_uid", "paymentRequestUid", "recurring_payment_uid"]);
  const stable = providerId ? String(providerId) : JSON.stringify(body || {});
  return crypto.createHash("sha256").update(stable).digest("hex");
}

async function claimPayPlusCallback(eventId, db = getFuelPhysiqueFirestore()) {
  const ref = db.doc(`billingEvents/${eventId}`);
  try {
    await ref.create({ eventHash: eventId, status: "processing", createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp() });
    return { claimed: true, ref };
  } catch (error) {
    if (error?.code === 6 || error?.code === "already-exists") return { claimed: false, ref };
    throw error;
  }
}

async function completePayPlusCallback(ref) {
  await ref.set({ status: "completed", completedAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp() }, { merge: true });
}

async function handleCallback(body, headers, { claim = claimPayPlusCallback, complete = completePayPlusCallback } = {}) {
  const config = billingConfig();
  ...
  const claimResult = await claim(callbackEventId(body));
  if (!claimResult.claimed) return { uid: userReference.uid, duplicate: true };
  try {
    ...
    await complete(claimResult.ref);
    return { uid: userReference.uid, duplicate: false };
  } catch (error) {
    // Permit a signed provider retry when the authoritative subscription write
    // did not complete. The durable marker is not treated as a completed event.
    await claimResult.ref.delete().catch(() => {});
    throw error;
  }
}

module.exports = { PRO_PRICE_ILS, billingConfig, callbackEventId, claimPayPlusCallback, createCheckout, handleCallback, verifyPayPlusHash, serviceAccount, firebaseAccessToken };
